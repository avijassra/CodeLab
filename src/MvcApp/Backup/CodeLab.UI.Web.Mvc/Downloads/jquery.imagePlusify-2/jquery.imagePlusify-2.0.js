            ...
            if ($selector.length > 0) {
                ...
                if($('#' + plusifyStyle).length == 0) {
                    ...
                    $head.append('<style id="' + plusifyStyle + '"> div.' + albumClass + '{position: relative; float: left;  } ' + 
                        ...
                        'div.' + albumClass + ' div.' + imgDivClass + ' {position: relative; background-color: ' + opts.backgroundColor + '; padding: ' + opts.padding + '; border: ' + opts.border + '; ' + (broserAnim && broserAnim.support ?  broserAnim.prefix + 'transition:all 1s ease-in-out;' : '') + '} '+
                        '</style>');
